...
- The folder holds `{config.yaml, expect.yaml}` — plus, only for a config with a file
  supplement, the CSV it names (config-relative); place streaming recipes under
