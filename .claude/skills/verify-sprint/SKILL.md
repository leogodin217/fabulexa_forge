...
2. Architecture doc for the feature (e.g., `{package}/docs/architecture/pending/*.md`)
...
**Code navigation:** cclsp-first — follow `.claude/skills/worker-protocol.md` § Code Navigation (`find_definition`, `find_references`, `get_incoming_calls`/`get_outgoing_calls` for call chains, `get_hover`; all work under basedpyright). Reserve Grep for non-symbol text only.
