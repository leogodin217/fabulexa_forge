...
| [`derivations.md`](derivations.md) | The derivations layer — interpretive shared folds between the reader and the modes; the versioned-intervals, reference-resolution, row-state-events (two-scope: a change-scope set governing `u` event membership × an independently-scoped after-image projection), membership-events, state-at (horizoned + end-of-tape), membership-state-at, and record-index (`record_id` → `record_index`, horizoned + end-of-tape) residents plus the truncated-tape surface (`open_truncated_tape` — the emit as a producer slice at T, opened by the seam's `state()` and the dimensional windowed compile alike), the layer contract (purity / anti-weld / traceability / temporal honesty), the fold-vs-join-relation reading of the determinism rule, the single-branch guard |
...
| [`row-predicates.md`](row-predicates.md) | The config row-predicate grammar shared by the dimensional mode's five predicate surfaces (`source.filter` / `source.where` / `source.value` / `fk.where` / `derived.elapsed.other_where`), the source mode's two (`tables[].where` / `events.sources[].where`), and the streaming mode's two (`streams[].where` on both stream shapes) — a scalar-or-list value compiling to `=` or `IN` under one rendering authority in the shared SQL utilities, sidecar-resolved literal typing with no VARCHAR fallback, the well-formedness rule carried by the `PredicateValue` type, and the equality-and-set-membership operator boundary |
| [`selection-spine.md`](selection-spine.md) | The mode-neutral row-selection spine the source and streaming modes both narrow rows through — the fan-out-free, horizon-free `record_id`-producing relation over a kind's records spine (the parent lookup when the caller passes a membership unit's owner kind), the no-op-not-composed rule, the resolved `WhereEntry` boundary that keeps every gate and error identity with the calling mode, and the two-case out-of-domain notice whose structure is shared and whose wording is the caller's |
...
| [`value-rendering-elections.md`](value-rendering-elections.md) | The cross-mode value-rendering election surface — author-elected renderings of payload column values over one property-first `render:` map shared with the temporal family: `decimal` (DOUBLE → exact `DECIMAL(p, s)`, ties away from zero, loud overflow), `instant` (a payload BIGINT declared a sim-instant, rendered through the temporal vocabulary and the anchor), and `json_precision` (named top-level JSON leaves rounded in place, every other byte preserved, via the reader-registered `forge_json_precision` scalar); per-mode attach points (source/base render maps, dimensional's derived pair, streaming's per-stream numeric map), the source event-log election-reach rule (`ElectionKindConflict`, raw-value changeset diffs), and the one-authority-per-election determinism statement |
...
| [`companion-artifacts.md`](companion-artifacts.md) | The companion artifacts — the README + manifest pair every file-writing export invocation deposits beside its datasets: the `<prefix>-readme.md` / `<prefix>-manifest.json` naming and placement, the writing rules (post-data, overwrite-unconditional, whole-state rewrite per emitting incremental window), the author `readme_overlay` H2-slot grammar (`overview` / `table: <name>`, exact-match, plan-time drift alarm), the README ordering contract over its three inputs (packaged mode template × overlay × derived facts), the pinned-byte-form manifest transcribed from the materialized Arrow schema, and the determinism / sourced-values / inertness invariants |
| [`documentation-channel.md`](documentation-channel.md) | The documentation channel — the bundle's five documentation surfaces (per-column `description`/`unit`, `tables[].description`, `enum_domains` glosses, `scenario_description`, the contract's pinned structural-column strings) forwarded end to end: the single-source column-inheritance rule with its author-first per-column `description` override, the author-first table-level `description` override (both attached in each companion-writing mode's addressing idiom), the forge-pinned event-log table + column documentation over the source compiler's `event_log` marker, plan-compile provenance carriage (`ColumnProvenance` / `KindValueEntry` / the author-description map and table-level facts on `QuerySpec` / `TableReport`, forwarded by both report-assembly sites), the companion README/manifest data dictionary, the YAML-comment `init` annotations all three proposal engines share, and the corrupter base-emit writer's attribute forwarding — all over the reader's `Sidecar.documentation()` view |
| [`supplements.md`](supplements.md) | Supplementary tables — the `supplements` block on `ExportConfig` (dimensional only): author-supplied tables carried verbatim into the export as ordinary tables, the manually-entered-warehouse-data case. The declaration (`SupplementDecl`: name, ordered typed `columns`, exactly one of `file` / `rows`, presentation prose), the loader (`load_supplements` — CSV read once, hashed, header-checked, resolved to text rows beside the config like `readme_overlay`), the pinned type vocabulary and the verbatim DuckDB `VARCHAR` cast, the `VALUES` relation compiled to a `QuerySpec` with its three pre-write gates (reserved name, `TIMESTAMPTZ` anchor, `TRY_CAST` cell probe), the source-is-output gate over the sites' pure naming functions, and the per-consumer contract (dimensional append, incremental `snapshot` + fingerprint, shaped playback, manifest `tables[].supplement`, pack members) |
| [`incremental.md`](incremental.md) | The incremental export driver — `--next` / `--from` / `--to` window-at-a-time export over the dimensional, source, and base modes: calendar/sim-time regimes, the dimensional horizon compile (the full export over the truncated tape at the window's two horizons, delivered per table as `snapshot` / `upsert` / `append` by a static delivery class, reconciled by the declared key; `KeyColumnsStable` and the reserved table-name rule always-on at load so no static windowed-only refusal exists, `WindowKeyDuplicate` the one data guard; the compile over a table selection and the horizon economy — the start horizon opened only for a delta-bearing compile), the cursor + fingerprint, drained detection |
| [`playback.md`](playback.md) | The playback seam — the caller-driven, pull-only, deterministic library surface driving an emit as a tape: tier 1 (primitive `events` / `snapshot` / `seek` over atom populations, below the modes, with a per-selection identity projection over the published maps) and tier 2 (shaped `window` / `state` over a declared target shape, above the modes — `window()` the dimensional horizon compile per delivery class, `state()` the full export over the truncated tape; per-ask `tables` selection on both — a projection over the whole-shape compile with projection invariance, atomic asks, guard locality, and static completeness at open — and the horizon economy), one inclusive-T event-time line, the consistency algebra, the canonical order + entry-point-invariant `seq`, the truncated-tape `state` compile, permissive totality |
| [`stream-playback.md`](stream-playback.md) | The stream-shaped playback head + render surface — `StreamConfig`-bound replay of the streaming exporter's event feed: bounded `events` (pure row selection, entry-point-invariant `seq`), `seek` (Debezium snapshot-then-stream — the `r` phase over records live at T, compaction semantics, the seek-state equivalence), `topics()`, and the pure per-event `StreamRender` (body/key bytes, epoch-ms record timestamp, per-event `(topic, table-identity)` value-schema resolution) — the seam's one byte-producing contract, consumed by the `stream` delivery driver (the driver adds no bytes) |
...
| [`datasets.md`](datasets.md) | The dataset-distribution surface — real producer-emitted example datasets (bundle + authored configs) as self-contained packs on GitHub Releases, named by a manifest baked into the wheel: the authored-allowlist manifest models + offline loader, `fabulexa-forge datasets list` / `get` (anonymous HTTPS fetch, sha256 + size + member-safety verification, failure-atomic extraction, printed next-step commands), the deterministic repo-side pack builder (stamp-from-the-pack, print-never-edit, `open_emit` as publishability gate), and the version-agreement-by-construction hygiene rules |
...
| `playback/` | The playback seam. Tier 1 (primitive) — `open_playback` / `Playback` / `PlaybackEvent` / `PlaybackSnapshot` / `PlaybackPosition` over atom selections, below the modes; imports the reader, derivations, anchor, `errors`. Tier 2 (shaped) — `open_shaped_playback` / `ShapedPlayback` / `ShapedTable` over a declared `ExportConfig` shape, above the modes; imports `config`, the modes' pure compile surfaces, the notice channel, and the derivations truncated-tape surface. The stream-shaped sibling — `open_stream_playback` / `StreamPlayback` (`stream.py`) and `resolve_stream_render` / `StreamRender` (`stream_render.py`) over a `StreamConfig`; imports the streaming exporter's pure compile/render surfaces, never its driver. Pull-only, deterministic, permissive; one inclusive-T event-time line. See [`playback.md`](playback.md), [`stream-playback.md`](stream-playback.md). | Implemented |
...
| Incremental export (`incremental` config, `--next` / `--from` / `--to`, cursor; horizon windowing over the truncated tape for the dimensional and source modes — per-table `snapshot` / `upsert` / `append` delivery classes, keyed reconciliation, the acceptance suite `tests/incremental/test_horizon_acceptance.py`) | Implemented (cross-mode driver over dimensional + source + base) — [`incremental.md`](incremental.md) |
...
| Source exporter — `mode: source` declared-table grammar (author-named `state` / `junction` tables + one polymorphic event log keyed by a dense tape-anchored `id` publishing the log's total order), per-table `columns`/`rename`, operational presentation defaults, mandatory wallclock anchor, corrupt→source composition, cross-mode incremental composition (the horizon compile: `state` / `junction` snapshots, the appended event-log delta), `init --mode source` | Implemented (Stage 3) — [`source.md`](source.md) |
...
| Playback seam — tier 1 (`open_playback` / `events` / `snapshot` / `seek` over atom selections; `PlaybackEvent`, entry-point-invariant `seq`, the consistency algebra) and tier 2 (`open_shaped_playback` / `window` / `state` over a declared `ExportConfig` shape; `window()` the dimensional horizon compile, `state()` the truncated-tape compile via `base_relations`; per-ask `tables` selection on both with the three `PlaybackError` gates, projection invariance, atomic asks, and the start horizon opened only for a delta-bearing selection); one inclusive-T event-time line, pull-only, deterministic, permissive; `last_mutation_sim_time` reserved-output-name posture; the `base_relations` indirection (dimensional compile parameter; post-compile rewrite for source) | Implemented — [`playback.md`](playback.md) |
| Stream playback — the stream-shaped head (`open_stream_playback` / `StreamPlayback`: `topics` / bounded `events` / `seek` with the `r` snapshot phase, `StreamEvent.op` admitting `r`) + the pure per-event render surface (`resolve_stream_render` / `StreamRender`: body/key bytes, epoch-ms timestamps, per-event `(topic, table-identity)` value-schema resolution + the run-level `value_schemas` enumeration under the same key), the engine's `resolve_streams` / `iter_resolved_stream_events` / `iter_resolved_snapshot_events` split, and the `stream` verb delivered over head + render (the driver adds no bytes) | Implemented — [`stream-playback.md`](stream-playback.md) |
...
| Config row predicates — the scalar-or-list grammar over the dimensional mode's five predicate surfaces, the source mode's two, and the streaming mode's two, the `render_predicate_condition` authority in the shared SQL utilities (the two private typed-literal forks consolidated into it), the `PredicateValue` well-formedness rule, per-element unobserved-value notices, and the dim source population set as the discriminator conjunct's selected subset | Implemented — [`row-predicates.md`](row-predicates.md) |
| Selection spine — the mode-neutral `build_selection_spine_sql` parent lookup (`WhereEntry`, `needs_population_filter`, `where_predicate_elements`, `check_where_values_observed`) promoted out of the source renders into a shared home, composed by source's owner-keyed narrowing and streaming's row scope alike | Implemented — [`selection-spine.md`](selection-spine.md) |
| Streaming authoring parity — bare-name after-image output keys + per-stream `rename` under one naming authority (`resolve_stream_output_columns` / `resolve_membership_output_columns`, read by the after-image assembly and the Debezium schema build alike), the kind vocabulary (`StreamConfig.kind_labels` + per-stream `kind_label`, identity fall-through, masquerade refusal), constant-gated row selection (`streams[].where` on both shapes, membership owner `sub_types`) resolved through the shared spine with the per-element unobserved-value notice, author-narrowed change scope (`only` / `ignore`) over the fold's audited set, the required `notice_sink` on `iter_stream_events` / `seed_mixer_run`, and the addressed-owner-set membership key-uniformity granularity | Implemented — [`streaming.md`](streaming.md) § Output names and `rename`, § Kind vocabulary, § Row selection, § Change scope |
| Temporal rendering elections — `date` / `time` / `timestamptz` instant renderings and `interval` elapsed rendering via `render_anchor_temporal_expr` (the generalized single shared renderer), a declared VARCHAR→temporal parse over the instant-string family via `render_date_parse_expr` (the closed date + time-of-day directive vocabulary validated by `validate_date_parse_format`, the `DATE` / `TIME` / naive `TIMESTAMP` output derived by the sole authority `date_parse_denoted_type`), the reader's session-zone pin, pinned CSV text forms for the elected types, the `TemporalRenderRequiresAnchor` / `DateParseSourceColumn` / `RenderKeyResolves` business rules, and per-mode attach points on the dimensional, source, and base exporters (including the election-aware ordinal amendment and the `KeyColumnsStable` key rule) | Implemented — [`temporal-elections.md`](temporal-elections.md) |
| Value rendering elections — the unified property-first `render:` map on source declared tables and base render entries (temporal shorthand + `date_parse` + `instant` + `decimal` + `json_precision`, one map so a conflicting pair is unrepresentable), dimensional's `derived: decimal` / `derived: json_precision`, streaming's per-stream numeric `render:` map with codec-seam after-image application, the two rendering authorities (`render_decimal_expr`, `render_json_precision_expr` over the reader-registered `forge_json_precision` scalar), the source event-log election reach (`ElectionKindConflict`, raw-value changeset diffs, pinned in-JSON temporal text), the source-type gates (`DecimalSourceIsDouble` / `InstantSourceIsBigint` / `JsonPrecisionSourceIsVarchar`), the pinned `DECIMAL` CSV text form, and compare's decimal canonical family | Implemented — [`value-rendering-elections.md`](value-rendering-elections.md) |
| SCD-2 per-version value renderings — the pure per-row value renderings (`derived: timestamp` / `date_parse` / `value_map` / `decimal` / `json_precision`) on an `scd: type2` dim, compiled through the shared per-column builders bound per source class (the records relation for untracked sources, the versioned reconstruction's cast value for tracked ones), gated by `Scd2ColumnModeSupported`; election-invariant version structure, source-class-blind rendering | Implemented — [`dimensional.md`](dimensional.md) § SCD-2 wide reconstruction |
...
| Dataset distribution — `fabulexa-forge datasets list` / `get` (offline listing, anonymous HTTPS fetch + sha256/size/member-safety verification + failure-atomic extraction), the wheel-shipped manifest (authored allowlist, builder-stamped fields, version-agreement hygiene test), the deterministic repo-side pack builder | Implemented — [`datasets.md`](datasets.md) |
| Companion artifacts — `readme_overlay` config + H2-slot overlay grammar (`load_readme_overlay` / `validate_overlay_tables`), the mode-neutral `write_companion_artifacts` writer over `ExportReport` / `TableReport` (all three file-writing engines + the incremental driver's whole-state rewrite), packaged per-mode README templates, the deterministic pinned-byte-form manifest, the census + fingerprint exclusions | Implemented — [`companion-artifacts.md`](companion-artifacts.md) |
| Documentation channel — `ColumnSpec` / `TableSpec` documentation attributes + the `Sidecar.documentation()` view (contract-pinned structural strings, enum glosses via the shared parse floor, scenario narrative), plan-compile provenance + author-description maps on `QuerySpec` / `TableReport`, the author-first per-column `description` override (dimensional column entry, source / base `descriptions` maps), the author-first table-level `description` override (dimensional / source table entries, base rename entries) + the forge-pinned event-log table/column set (`origin: "forge"`, the `event_log` marker), the companion README/manifest data dictionary, `init` YAML-comment annotations (dimensional / source / streaming), corrupter base-emit attribute forwarding | Implemented — [`documentation-channel.md`](documentation-channel.md) |
| Supplementary tables — `ExportConfig.supplements` / `SupplementDecl` (dimensional only), `load_supplements` → `ResolvedSupplement`, `compile_supplement_specs` (the `VALUES` relation, the reserved-name / anchor / cell-probe gates before any write), `check_supplement_sources_not_outputs` over `csv_output_paths` / `csv_removed_dirs` / `companion_artifact_paths` / `csv_cursor_path`, `snapshot` delivery + the fingerprint `supplements` map, shaped-playback table-set membership, the manifest `tables[].supplement` provenance, pack-builder members, the `supplement-tables` recipe | Implemented — [`supplements.md`](supplements.md) |
