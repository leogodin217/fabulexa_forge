...
| [`playback.md`](playback.md) | The playback seam — the caller-driven, pull-only, deterministic library surface driving an emit as a tape: tier 1 (primitive `events` / `snapshot` / `seek` over atom populations, below the modes, with a per-selection identity projection over the published maps) and tier 2 (shaped `window` / `state` over a declared target shape, above the modes), one inclusive-T event-time line, the consistency algebra, the canonical order + entry-point-invariant `seq`, the truncated-tape `state` compile, permissive totality |
