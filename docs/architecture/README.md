...
| Derivations layer + versioned-intervals / reference-resolution / row-state-events / membership-events / state-at (horizoned + end-of-tape) / membership-state-at residents + the truncated-tape surface + single-branch guard | Implemented (Stage 3, extended for playback) — [`derivations.md`](derivations.md) |
...
| Notice channel — `Notice` / `NoticeSink` / `render_notice_stderr`, required `notice_sink` on every emitting entry point, `slice-only-column-omitted` + `discriminator-value-unobserved` codes | Implemented — [`notices.md`](notices.md) |
