...
| Config row predicates — the scalar-or-list grammar over the dimensional mode's five predicate surfaces, the `render_predicate_condition` authority in the shared SQL utilities (the two private typed-literal forks consolidated into it), the `PredicateValue` well-formedness rule, per-element unobserved-value notices, and the dim source population set as the discriminator conjunct's selected subset | Implemented — [`row-predicates.md`](row-predicates.md) |
