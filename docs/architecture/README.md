...
| [`documentation-channel.md`](documentation-channel.md) | The documentation channel — the bundle's five documentation surfaces (per-column `description`/`unit`, `tables[].description`, `enum_domains` glosses, `scenario_description`, the contract's pinned structural-column strings) forwarded end to end: the single-source column-inheritance rule with its author-first per-column `description` override (the channel's one author input, attached in each companion-writing mode's column-addressing idiom), plan-compile provenance carriage (`ColumnProvenance` / `KindValueEntry` / the author-description map on `QuerySpec` / `TableReport`, forwarded by both report-assembly sites), the companion README/manifest data dictionary, the YAML-comment `init` annotations all three proposal engines share, and the corrupter base-emit writer's attribute forwarding — all over the reader's `Sidecar.documentation()` view |
