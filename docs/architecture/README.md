...
| Stream playback — the stream-shaped head (`open_stream_playback` / `StreamPlayback`: `topics` / bounded `events` / `seek` with the `r` snapshot phase, `StreamEvent.op` admitting `r`) + the pure per-event render surface (`resolve_stream_render` / `StreamRender`: body/key bytes, epoch-ms timestamps, per-event `(topic, table-identity)` value-schema resolution), the engine's `resolve_streams` / `iter_resolved_stream_events` / `iter_resolved_snapshot_events` split, and the `stream` verb delivered over head + render (the driver adds no bytes) | Implemented — [`stream-playback.md`](stream-playback.md) |
