...
| Incremental export (`incremental` config, `--next` / `--from` / `--to`, cursor; dimensional horizon windowing — per-table `snapshot` / `upsert` / `append` delivery classes over the truncated tape, keyed reconciliation, the acceptance suite `tests/incremental/test_horizon_acceptance.py`) | Implemented (cross-mode driver over dimensional + source + base) — [`incremental.md`](incremental.md) |
