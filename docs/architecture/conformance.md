...
contract, running the fifteen checks C1–C15 against an opened [`Emit`](reader.md). It
...
| Module | Owns |
|---|---|
| [`conformance.py`](../../src/fabulexa_forge/reader/conformance.py) | `validate`, `run_check`, `CheckResult`, `ConformanceReport`, the fifteen checks, the pinned spec (PS) column lists, the `to_csv_text` codec, identifier quoting |
...
- **Output.** A `ConformanceReport`: exactly fifteen `CheckResult`s in C1..C15 order.
- **Reads.** The vendored `contract/base-format.schema.json` (package data) for C1;
  ...
  C2–C13; the sidecar alone for C14 and C15.
...
returns exactly fifteen `CheckResult`s in C1..C15 order and never raises a
...
`validate` runs fifteen checks. Their assertions and independent-implementation notes:
...
| C15 | Surface consistency: the sidecar's `surface` discriminator is `"published"` and no `projection` block is present | **Sidecar-only, no data query.** Never skips. The other invariants of a published emit (no `firings` table, no machinery kinds, no provenance column group) are owned by C3, C12, and C5; C15 checks only what those do not — the discriminator itself and the paired `projection` absence |
...
| Condition | Result |
|---|---|
| All of C1–C15 pass | print per-check PASS summary (with any skips noted); exit 0 |
...
1. **Conformance independence.** `validate` reproduces C1–C15 (and the `to_csv_text`
...
3. **Total and never-raising.** `validate` returns exactly fifteen `CheckResult`s in
   C1..C15 order. A conformance failure is a failing `CheckResult`, never an
...
- **`validate` reimplements the full procedure C1–C15, which is itself narrower than
...
- **Pair agreement and index resolution are producer trust class.** The
  ...
  values against the target's `record_index` outside C1–C15 —
...
- **C11 and C13 (the temporal-attribute checks) ARE reimplemented** (semantic checks,
  ...
  they trust C1–C15), and C11/C13 are the checks that warrant that trust. The
...
| [`../../contract/base-format.md`](../../contract/base-format.md) § Conformance procedure | The vendored conformance procedure this reimplements (C1–C15), and § Format versioning behind the C1 carve-out |
