...
| `date_ref: {source} \| {from, format} \| {scd_window}` | Calendar key | An `INTEGER` `yyyymmdd` key into the generated `dim_date` — a structural instant's local date in the anchor zone, a parsed date string, or (on an `scd: type2` table only) the local date of the version-window bound ([`date-dimension.md`](date-dimension.md)) |
...
| `date_ref: {scd_window}` | The calendar key of a version bound's local date — the version relation's `version_start` / `version_end` through the `date` election and `date_key_expr`, the same input `derived: scd_window` reads ([`date-dimension.md`](date-dimension.md) § The `date_ref` column mode) |
| `derived: timestamp` / `date_parse` / `value_map` / `decimal` / `json_precision` / `date_ref` (instant or parse shape) | The rendering authority's output for the row surface's value — per version for a tracked source, per record otherwise |
...
| `TemporalRenderRequiresAnchor` | Every explicitly-elected instant rendering — `derived: timestamp`'s `as`, the `scd_window` object form, or a `date_ref` instant or bound shape (each inherently a `date` election) — has a resolved effective anchor; naming the column when it does not ([`temporal-elections.md`](temporal-elections.md)) |
...
| `DateRefWindowBoundRequiresScd2` | A bound-shape `date_ref` (`{scd_window: valid_from \| valid_to}`) declares on an `scd: type2` table — the version window it addresses exists only on a versioned dim. Always-on, full export included, a pure function of the declaration; runs in the per-column loop ahead of `TemporalRenderRequiresAnchor`, the one ordering that matters, since no other per-column rule reads the shape ([`date-dimension.md`](date-dimension.md) § Validation Rules). Error: `"column '{name}' on table '{table}': date_ref scd_window: {bound} addresses the SCD-2 version window, and the table is not scd: type2"`. Implementation: [`validation.py`](../../src/fabulexa_forge/exporters/dimensional/validation.py) `check_date_ref_window_bound_on_scd2` |
