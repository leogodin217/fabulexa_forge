...
| Recipe | What it teaches |
|---|---|
| [`corrupt/drift-and-duplicates`](../../examples/recipes/corrupt/drift-and-duplicates/config.yaml) | `duplicate_rows` with a `where` row filter (an exact-duplicate defect, C9) composed with `schema_drift`'s `rename_to` (a column rename the sidecar no longer describes, C11) — two operations threaded over the same shared working set |
