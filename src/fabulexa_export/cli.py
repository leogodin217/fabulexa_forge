"""CLI entry point for fabulexa_export.

Provides the `fabexport` command with the `validate`, `export`, `init`,
`stream`, `mixer`, and `corrupt` verbs.

Usage:
    fabexport validate <emit_dir>
    fabexport export <emit_dir> <config_path> <out> --fmt <csv|duckdb>
    fabexport init <emit_dir> [<out_path>]
    fabexport stream <emit_dir> <config_path> --fmt jsonl --sink <stdout|file>
        [--out <dir>] [--base-date <iso>] [--timezone <iana>]
    fabexport mixer <emit_dir> <config_path> --fmt jsonl|debezium
        --bootstrap-servers <addr> [--base-date <iso>] [--timezone <iana>]
        [--speed <n>] [--play|--paused] [--tick <s>]
        [--host <h>] [--port <p>]
    fabexport corrupt <emit_dir> --config <corrupt.yaml> --out <out_dir>
...
from fabulexa_export.reader import open_emit, validate
...
if TYPE_CHECKING:
    ...
    from fabulexa_export.corrupters.state import CorruptReport
    from fabulexa_export.reader.conformance import CheckResult
...
def _dispatch_export(
    ...
    """
    if next_window:
        from fabulexa_export.incremental.driver import export_incremental_next
    ...
    if range_from is not None and range_to is not None:
        from fabulexa_export.incremental.driver import export_window
        from fabulexa_export.incremental.windows import parse_range
...
def cmd_export(
    ...
    """`fabexport export` — full, next-window, or explicit-range export.
...
def cmd_init(emit_dir: Path, out_path: Path | None) -> int:
    """`fabexport init` — emit a commented candidate dimensional config.
    ...
    Returns:
        Process exit code.
    """
    from fabulexa_export.exporters.dimensional.init import generate_init_config
...
def cmd_stream(
    ...
    """`fabexport stream` — replay the base layer as a CDC event stream.
    ...
    from fabulexa_export.exporters.streaming.pacer import resolve_clock
...
def cmd_mixer(
    ...
    """`fabexport mixer` — replay the base layer as a live, operator-mixable Kafka feed.
    ...
    from fabulexa_export.exporters.streaming.driver import build_kafka_render_value
...
def cmd_corrupt(emit_dir: Path, config_path: Path, out_dir: Path) -> int:
    """`fabexport corrupt` — apply a corrupter config to an emit.
...
def main(argv: list[str] | None = None) -> int:
    """Entry point for the fabexport CLI.
