"""Export-pipeline exception hierarchy for fabulexa_export.
...
class MixerExtraUnavailable(ExporterError):
    """`fabexport mixer` was invoked but FastAPI / the ASGI server is not importable —
    the optional `mixer` install extra is absent. The message names the fix (install the
    extra). A direct child of ExporterError (sibling of KafkaClientUnavailable); caught
    by the CLI's (ReaderError, ExporterError) funnel as exit 1."""
...
class CorruptError(ExporterError):
    ...
    errors), so `fabexport corrupt` catches it under the same
