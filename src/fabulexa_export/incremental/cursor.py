...
inside the window's transaction). CSV: cursor lives in out/.fabexport-cursor.json.
"""
...
from fabulexa_export.errors import ExportRuntimeError, IncrementalCursorInvalid

_CURSOR_FILE = ".fabexport-cursor.json"
...
def _parse_csv_cursor_file(cursor_file: Path) -> Cursor:
    ...
    Args:
        cursor_file: Path to .fabexport-cursor.json.
...
def read_cursor(
    ...
    out/.fabexport-cursor.json with keys exactly the Cursor field names.
...
def write_csv_cursor(out: Path, cursor: Cursor) -> None:
    """Write out/.fabexport-cursor.json (keys = Cursor field names).
