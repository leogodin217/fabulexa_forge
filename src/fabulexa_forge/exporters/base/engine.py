...
def build_base_query_specs(
    ...
    Returns:
        ...
        Every spec's `provenance`, `author_descriptions`, and
        `author_table_description` are copied verbatim from its
        `BaseTableSpec` (stamped at plan build); `kind_values` stays empty —
        base carries no kind-name-as-value column.
    ...
    for table_spec in plan.tables:
        ...
        specs.append(
            QuerySpec(
                ...
                author_descriptions=table_spec.author_descriptions,
                author_table_description=table_spec.description,
            )
        )
    return specs
