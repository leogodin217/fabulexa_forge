...
def build_base_query_specs(
    ...
    Returns:
        ...
        Every spec's `provenance` and `author_descriptions` are copied
        verbatim from its `BaseTableSpec` (stamped at plan build);
        `kind_values` stays empty — base carries no kind-name-as-value
        column.
