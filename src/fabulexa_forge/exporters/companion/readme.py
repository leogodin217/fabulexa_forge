...
def _render_title(
    ...
    The emit's `scenario_name` (when present) titles the export after the
    mode; absent, the title is the mode alone.
    """
    title = f"# {_MODE_TITLES[mode]} Export"
    if scenario_name:
...
def render_readme(
    ...
    sections = [_render_title(mode, manifest_filename, doc.scenario_name())]
