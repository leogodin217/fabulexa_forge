...
read no data. `ShapedPlayback.tables()` (this phase) reports the shape's
declared output tables and their static per-class/per-genre window-delivery
class; `.window()` (Phase 11) and `.state()` (Phase 12) land later.
...
`exporters.notices`, the dimensional validation module, the source plan module,
